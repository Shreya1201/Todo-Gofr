...
function removeItem() {
  ...
  const url = "http://localhost:8000/todo/delete?" + params.toString();
...
function completeItem() {
  ...
  const params = new URLSearchParams();
  params.append("id", taskId);
  params.append("status", id === "todo");
  const url = "http://localhost:8000/todo/update?" + params.toString();
...
function sendTaskToAPI(item, callback) {
  ...
  const url = "http://localhost:8000/todo/add?" + params.toString();
...
function getTasks(callback) {
  var req = new XMLHttpRequest();
  req.open("GET", "http://localhost:8000/todos");
  req.send();

  req.addEventListener("load", () => {
    var results = JSON.parse(req.responseText).data;
    console.log(results);
    if (results.error) return console.log(results.error);
